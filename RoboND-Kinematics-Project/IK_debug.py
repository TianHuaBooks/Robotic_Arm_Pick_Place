...
class CalcObject:
   ...
   def __init__(self):
	...
	#
        ...
        ROT_y = Matrix([[cos(self.p),    0,      sin(self.p)],
                        [0,         1,           0],
                        [-sin(self.p),   0,      cos(self.p)]]) #Pitch
        ROT_z = Matrix([[cos(self.y),  -sin(self.y),  0],
                        [sin(self.y),   cos(self.y),      0],
                        [     0,        0,      1]]) #Yaw
        ROT_EE = ROT_z * ROT_y * ROT_x
        Rot_Error = ROT_z.subs(self.y, np.pi) * ROT_y.subs(self.p, -np.pi/2)
...
if __name__ == "__main__":
	...
	print("***testing {}".format(test_case_number))
